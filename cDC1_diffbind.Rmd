...
```{r}
cDC1 <- dba(sampleSheet="cDC1_diffbind.csv")
```
...
```{r}
sig_results <- dba.report(cDC1, method=DBA_EDGER, th=0.01, bUsePval=TRUE)
```
...
```{r}
# Export separately
export.bed(gained, "cDC1_gained_accessibility.bed")
export.bed(lost, "cDC1_lost_accessibility.bed")
```
...
```{r}
write.table(combined_bed, "cDC1_significant_peaks_labeled.bed", 
            quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
```

```{r}
cat("\nExported files:\n")
cat("- cDC1_gained_accessibility.bed:", length(gained), "peaks\n")
cat("- cDC1_lost_accessibility.bed:", length(lost), "peaks\n")
cat("- cDC1_significant_peaks_labeled.bed: combined file\n")
```

```{r}
export.bed(sig_sorted, "cDC1_significant_peaks_sorted.bed")
```
...
all_results <- dba.report(cDC1, method=DBA_EDGER, th=1, bUsePval=FALSE)
...
write.csv(peaks_all_df, "cDC1_diffbind_ALL_annotated.csv", row.names = FALSE)

# Save unique genes only (for RNA-seq integration)
write.csv(peaks_for_rna, "cDC1_diffbind_ALL_annotated_unique_genes.csv", row.names = FALSE)

cat("\nExported:\n")
cat("- cDC1_diffbind_ALL_annotated.csv (all peaks, includes genes with multiple peaks)\n")
cat("- cDC1_diffbind_ALL_annotated_unique_genes.csv (one peak per gene, for RNA integration)\n")
```
...
write.csv(peaks_sig_df, "cDC1_diffbind_significant_annotated.csv", row.names = FALSE)

cat("- cDC1_diffbind_significant_annotated.csv (only significant peaks)\n")
```
